...
ARAopen,Rdec  = sym.symbols('ARAopen, Rdec')
AROopen, ORAopen = sym.symbols('AROopen, ORAopen')
D = sym.Symbol('D')
...
class PostSynapticModel:
    ...
    def __init__(self, kinet=None, pnt=None):
        ...
        if kinet:
            ...
            self.k_dissA_ast  = kinet['k_dissA_ast'] *Tbase 
            self.k_assocA_ast = kinet['k_assocA_ast'] *R_base*Tbase
    ...
    def _formulate_ODE(self):
        ...
        k_open    = self.k_open
        k_close   = self.k_close
        k_dpls   = self.k_dpls
        k_dmns   = self.k_dmns
        k_dissA_ast   = self.k_dissA_ast
        k_assocA_ast  = self.k_assocA_ast
        
        ORO = R_total - (ARO+ORA+ARA+DRO+ORD+DRD+ARD+DRA+ARAopen+AROopen+ORAopen+Rdec)
        dxdt = {A: k_dissA1*(ARA+ARO+ARD) + k_dissA2*(ARA+ORA+DRA) - k_hydrol*A \
                   ...
                   + k_dissA_ast*ARAopen - k_assocA_ast*A*ORAopen \
                   + k_dissA_ast*ARAopen - k_assocA_ast*A*AROopen,
                ...
                ARAopen: k_open*ARA \
                     -k_dissA_ast*ARAopen +k_assocA_ast*A*ORAopen \
                     -k_dissA_ast*ARAopen +k_assocA_ast*A*AROopen \
                     + k_dmns*Rdec  - k_dpls*ARAopen,
                AROopen: k_dissA_ast*ARAopen -k_assocA_ast*A*AROopen -k_close*AROopen,
                ORAopen: k_dissA_ast*ARAopen -k_assocA_ast*A*ORAopen -k_close*ORAopen,
                Rdec : -k_dmns*Rdec  + k_dpls*ARAopen
                }                
        
        self.ivp = InitialValueProb((A, ARO, ORA, ARA, DRO, ORD, DRD, DRA, ARD, D, ARAopen, AROopen, ORAopen, Rdec), t)
    ...
    def peak_Ropen(self):
        ...
        ARAの濃度のピーク値を取得します
        """
        ARA_timecourse = self.ivp.get_timecourse(ARAopen)
        ARO_timecourse = self.ivp.get_timecourse(AROopen)
        ORA_timecourse = self.ivp.get_timecourse(ORAopen)
        timecourse = ARA_timecourse+ARO_timecourse+ORA_timecourse       
...
def estimate_ara50_gamma(model, occ50, Twmax, initACh):
    ...
    ARA50 = model.peak_Ropen() * R_base    
    #print(ARA50)

    #　ARAmaxを用いてgammaを計算
    model.set_steady_state(0)
    model.solve_competition_problem(time=5, initACh=initACh)
    ARAmax = model.peak_Ropen() * R_base    
...
if __name__ == "__main__":
    ...
    def write_graph(sol, filename):
        ...
        plt.plot(sol.t, (sol.y[10]+sol.y[11]+sol.y[12])*300, label='R*', ls='-', lw=3, c='k')
        #plt.plot(sol.t, sol.y[10]*100, label='ARAopen', ls='-', lw=3, c='c')
